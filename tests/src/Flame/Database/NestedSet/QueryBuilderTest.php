...
namespace Igniter\Tests\Flame\Database\NestedSet;
...
it('lists dates', function() {
    $statusHistory1 = StatusHistory::factory()->create(['updated_at' => '2020-01-01']);
    $statusHistory2 = StatusHistory::factory()->create(['updated_at' => '2020-03-02']);

    $dates = StatusHistory::query()->pluckDates('updated_at');
    $statusHistory1 = $statusHistory1->fresh();
    $statusHistory2 = $statusHistory2->fresh();

    expect($dates)->toHaveKey($statusHistory1->updated_at->format('Y-m'), $statusHistory1->updated_at->format('F Y'))
        ->and($dates)->toHaveKey($statusHistory2->updated_at->format('Y-m'), $statusHistory2->updated_at->format('F Y'));
});
