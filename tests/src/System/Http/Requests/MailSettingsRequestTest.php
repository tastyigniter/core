...
it('returns correct validation rules', function() {
    ...
    expect($rules['sender_name'])->toContain('required', 'string')
        ...
        ->and($rules['smtp_host'])->toContain('string')
        ->and($rules['smtp_port'])->toContain('string')
        ->and($rules['smtp_user'])->toContain('string')
        ->and($rules['smtp_pass'])->toContain('string')
        ->and($rules['mailgun_domain'])->toContain('required_if:protocol,mailgun', 'string')
        ->and($rules['mailgun_secret'])->toContain('required_if:protocol,mailgun', 'string')
        ->and($rules['postmark_token'])->toContain('required_if:protocol,postmark', 'string')
        ->and($rules['ses_key'])->toContain('required_if:protocol,ses', 'string')
        ->and($rules['ses_secret'])->toContain('required_if:protocol,ses', 'string')
        ->and($rules['ses_region'])->toContain('required_if:protocol,ses', 'string');
});
