@foreach ($records ?? [] as $theme)
    @unless ($theme->getTheme())
        {!! $self->makePartial('themes/not_found', ['theme' => $theme]) !!}
    @else
        <div class="row mb-3">
            <div class="d-flex align-items-center bg-light p-4 w-100">
                @if ($theme->getTheme()->hasParent())
                    {!! $self->makePartial('themes/child_theme', ['theme' => $theme]) !!}
                @else
                    <a
                    ...
                    <div class="media-body">
                        ...
                        <div class="list-action align-self-end my-3">
                            {!! $self->makePartial('lists/list_buttons', ['theme' => $theme]) !!}
                        </div>
                    </div>
                @endif
            </div>
            @if (strlen($theme->screenshot))
                {!! $self->makePartial('themes/screenshot', ['theme' => $theme]) !!}
            @endif
        </div>
    @endunless
@endforeach
