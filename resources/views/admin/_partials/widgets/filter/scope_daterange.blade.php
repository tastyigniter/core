...
>
    <div class="input-group">
        ...
        <input
            type="text"
            id="{{ $self->getScopeName($scope) }}-daterangepicker"
            class="form-control"
        ...
        >
        <input data-datepicker-range-start type="hidden" name="{{ $self->getScopeName($scope) }}[]" value="{{ $scope->value[0] ?? '' }}">
        <input data-datepicker-range-end type="hidden" name="{{ $self->getScopeName($scope) }}[]" value="{{ $scope->value[1] ?? '' }}">
    </div>
</div>
