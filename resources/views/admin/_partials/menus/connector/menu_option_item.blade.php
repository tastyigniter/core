    ...
    <div class="px-2">
        <i
            ...
            title="{{ $item->isRequired()
                ? lang('igniter::admin.menu_options.is_required')
                : lang('igniter::admin.menu_options.is_not_required') }}"
        ></i>
    </div>
    <div class="px-2">
        @if($item->display_type == 'radio')
            <i
                title="{{ sprintf(lang('igniter::admin.menu_options.text_option_summary'), $item->display_type) }}"
        ...
        @elseif($item->display_type == 'quantity')
            ...
            ></i>
        @else
            {{ sprintf(lang('igniter::admin.menu_options.text_option_summary'), $item->display_type) }}
        @endif
    </div>
    <div class="px-2">
