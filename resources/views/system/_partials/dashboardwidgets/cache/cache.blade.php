<div id="{{ $self->getId() }}" class="dashboard-widget widget-cache">
    <h6 class="widget-title">@lang($self->property('title'))</h6>

    <span>@lang('igniter::admin.dashboard.text_total_cache')<b>{{ $formattedTotalCacheSize }}</b></span>
    ...
    <button
        type="button"
        data-request="{{ $self->getEventHandler('onClearCache') }}"
        data-request-success="$('#cache-sizes').replaceWith(data.partial)"
        class="btn btn-default"
    ><i class="fa fa-trash"></i>&nbsp;&nbsp;@lang('igniter::admin.text_clear')</button>
</div>
