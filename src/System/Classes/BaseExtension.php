...
class BaseExtension extends ServiceProvider
{
    ...
    protected function getConfigFromFile()
    {
        ...
        $configPath = realpath(dirname(File::fromClass($className), 2));
        $extensionCode = strtolower(str_replace('/', '.', dirname(str_replace('\\', '/', $className))));

        if (!$config = array_get(resolve(ComposerManifest::class)->extensions(), $extensionCode)) {
