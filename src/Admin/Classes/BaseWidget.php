...
class BaseWidget extends Extendable
{
    ...
    public function __construct($controller, $config = [])
    {
        ...
        $vendor = str_replace('/', '.', $relativePath);
        $path = str_after($classPath, $relativePath.'/');
        $this->partialPath[] = $vendor.'::_partials.'.$path;
        $this->partialPath[] = $vendor.'::_partials.'.dirname($path);
        $this->partialPath[] = $vendor.'::_partials';

        $this->assetPath[] = '@/js/'.dirname($path);
        $this->assetPath[] = '@/css/'.dirname($path);
        $this->assetPath[] = '@/js';
        $this->assetPath[] = '@/css';
