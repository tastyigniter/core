...
use Igniter\System\Classes\Controller;
...
class AdminController extends BaseController
{
    ...
    use \Igniter\System\Traits\VerifiesCsrfToken;
    ...
    protected function definePaths()
    {
        ...
        $vendor = str_replace('/', '.', dirname($classPath, 2));

        // Add paths from the extension / module context
        $this->viewPath[] = $vendor.'::'.$className;
        ...
        $this->partialPath[] = $vendor.'::_partials.'.$className;
        $this->partialPath[] = 'igniter.admin::_partials.'.$className;
        $this->partialPath[] = $vendor.'::_partials';
        $this->partialPath[] = 'igniter.admin::_partials';

        $this->configPath[] = '@/models/admin';
        $this->configPath[] = '@/models/main';
        $this->configPath[] = '@/models/system';
    ...
    public function remap($action, $params)
    {
        ...
        if (!$this->verifyCsrfToken()) {
            return Response::make(lang('igniter::admin.alert_invalid_csrf_token'), 403);
        }
        ...
        if ($requireAuthentication) {
            if (!$this->checkUser()) {
                return Request::ajax()
                    ? Response::make(lang('igniter::admin.alert_user_not_logged_in'), 403)
    ...
    protected function runHandler($handler, $params)
    {
        ...
        if (strpos($handler, '::')) {
            ...
            if (!isset($this->widgets[$widgetName])) {
                throw new Exception(sprintf(lang('igniter::admin.alert_widget_not_bound_to_controller'), $widgetName));
            }
